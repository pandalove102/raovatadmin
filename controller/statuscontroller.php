...
{	
	...
	function index()
	{
		...
		{
			$status = $this->model->searchstatus($this->get('key'),$this->tim_vi_tri_bat_dau($this->numrow),$this->numrow);
			if($this->get('key') !='')
			{
				$totalpage =count($status);
			}else{
				$totalpage = $this->model->total();
			}
		}else{
			$status = $this->model->liststatus($this->tim_vi_tri_bat_dau($this->numrow),$this->numrow);
			$totalpage = $this->model->total();
		}
		$this->setdata(array('status'=>$status,'totalpage'=>$totalpage));
