...
{	
	...
	function index()
	{
		...
		{
			$news = $this->model->listnewssearch($this->tim_vi_tri_bat_dau($this->numrow),
												$this->numrow,
												$this->get('key'),
												$this->get('catagories_id'),
												$this->get('status')
												);
			if($this->get('key') !=''||
			$this->get('catagories_id')!=''||
			$this->get('status')!='')
			{
				$totalpage=count($news);
			}else {
				$totalpage = $this->model->total();
			}	
		}else
		{
			$news = $this->model->listnews($this->tim_vi_tri_bat_dau($this->numrow),$this->numrow);
			$totalpage = $this->model->total();
		}
		$catagories=$this->models->listcatagoriesall($this->tim_vi_tri_bat_dau($this->numrow),$this->numrow);
