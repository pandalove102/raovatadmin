...
{	
	...
	function index()
	{
		$this->title='Danh sách nhóm bài viết';
		$this->h = 'Danh sách nhóm bài viết';
		$this->a = 'nhóm bài viết';
		$this->strong = 'Danh sách';
		if($this->get() && $this->istoken('tokensearch',$this->model->clean($this->get('tokensearch'))))
		{
			$catagories = $this->model->listcatagoriessearch($this->tim_vi_tri_bat_dau($this->numrow),
															 $this->numrow,
															 $this->get('key'));
			if($this->get('key') !='')
			{
		...
		{
			$catagories = $this->model->listcatagories($this->tim_vi_tri_bat_dau($this->numrow),$this->numrow);
			$totalpage = $this->model->total();
		}
	...
	function create()
	{
		$this->title ='Thêm nhóm bài viết';
		$this->h = 'Thêm nhóm bài viết';
		$this->a = 'nhóm bài viết';
		$this->strong = 'Thêm';
		$this->save = 'Lưu';
		...
		$this->setdata(array('list_catagories'=>$this->model->listcatagoriesall()));
		...
		{			
			...
			{ 
				...
				{
					$this->model->logs('Thêm thành công nhóm bài viết: '.$this->model->clean($this->post('username')),$this->getcontrollername().'/'.$this->uri);
			...
			{
				$this->setmsg('Tên nhóm bài viết đã tồn tại!','error');
			}
		}
	...
	function delete()
	{
		...
		if($this->model->update($data))
		{
			$this->model->logs('Xóa thành công nhóm bài viết: '.$catagories->name,'catagories/delete/'.$catagories->id);
			redirect('catagories',2);
		}
		else
		{
